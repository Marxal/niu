...
## Round 17.2 — Fix: the buttons didn't answer, and the badge was a blank blob

**Same branch, testing round 17.1 on a real phone.** Marçal: "the notification
has YES or Can't but tapping yes or can't doesn't take effect, it simply opens
the app," and separately, "there's a small white bubble on the left, and also
on the top of the phone" where Niu's icon should be.

**Redeploy `niu-push` once more** — no new secret this time. See the fix note
under Round 17.1 in `docs/SUPABASE_SETUP.md`.

### Bug 1: CORS

`/confirm` is called by a phone's browser, not by Postgres — the first route
in this function to ever be called from one. A browser refuses a cross-origin
request unless the server explicitly permits it (CORS), and Supabase doesn't
add that permission on its own. Every tap of Yes or Can't was failing before
the request even left the phone — a plain network error, indistinguishable
from having no signal — which tripped the exact fallback built for that case:
open the app instead of leaving the tap looking like nothing happened. The
fallback was doing its job; the job just shouldn't have been needed every
time.

Fixed by answering the browser's preflight check and echoing an
`Access-Control-Allow-Origin` header back on every response from `/confirm`,
restricted to Niu's own two origins — the same `https://marxal.github.io` and
`http://localhost:5173` already listed as "Authorized JavaScript origins" for
Google Calendar, for the identical reason. The trigger route stays exactly as
it was; a server calling a server was never subject to this in the first
place.

### Bug 2: the icon

Every existing app icon (`icon-192.png` and the rest) is a fully opaque,
full-colour image — correct for the home screen, where a launcher needs a
solid background to safely crop it into a circle or squircle. A notification
badge on Android draws from the opposite property: it reads only the image's
*transparency* and paints whatever is opaque as a plain white shape. Handed a
fully opaque square, there is nothing to trace — hence the blob.

`scripts/generate-icons.mjs` now also writes `public/icons/badge-96.png`: a
mostly-transparent 96×96 PNG containing just the nest-and-egg glyph, the same
mark `favicon.svg` already used before the photographic wordmark arrived (see
that script's own header) — a silhouette is exactly what a simple line mark is
for, and exactly what a photographic render is not. `public/sw.js` points both
`badge` fields at it. The four existing icons are regenerated byte-identical
by the same script run; nothing about the home-screen icon changes.

### How to test it

1. **Redeploy `niu-push`** with the current code.
2. Push this branch to `main` so the new service worker and badge ship —
   Yes/Can't and the icon are both client-side, unlike the function.
3. **Fully close Niu** on the phone that will receive the notification —
   swiped away — then reopen it once, so the new service worker takes over.
4. Repeat the round 17.1 test: ask to confirm, tap **Yes** straight on the
