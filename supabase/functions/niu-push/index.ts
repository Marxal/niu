 ...
 *
 * ## Round 17.1 tried answering straight from the notification, and round 17.3
 * undid it
 *
 * Yes / Can't buttons on the notification, answering without opening the app,
 * shipped in round 17.1 and worked in testing. It did not work on Marçal's own
 * phone once he actually relied on it: tapping Yes reliably recorded "Can't" —
 * confirmed down to the raw payload this function received, so the phone's
 * browser itself was reporting the wrong button, before any code here ever
 * ran. A binary question that sometimes silently records the opposite answer
 * is worse than one that requires an extra tap, so the buttons are gone.
 *
 * A notification now only ever opens the app (see the service worker), which
 * lands on the calendar's own "waiting on you" card — the one place Yes/Can't
 * still lives, answered with a real session behind it.
 *
 ...
 *
 * pg_net queued its call and stopped caring, so an exception here reaches
 * nobody — every path returns 200 with a short note in the body, so the
 * function log reads as a history rather than a stack trace.
 *
 * ## Deploying it
 *
 * Supabase dashboard -> Edge Functions -> Deploy a new function, named
 * `niu-push`. Paste this file. Turn **Verify JWT off** — the caller is
 * Postgres, which has no user session and proves itself with the shared secret
 * instead. Then set three secrets:
 *
